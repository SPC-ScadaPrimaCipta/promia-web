...
/**
 * GET /api/risk-analysis/report-count-all-level-graphic
 *
 * Query Parameters:
 ...
 *
 * Returns:
 * - data: All rows from dbo.RPT_COUNT_ALL_LEVEL_GRAFIK for the given model and asset
 */
    ...
    try {
        ...
        const query = `
            SELECT
                ...
                equipment,
                cost
            FROM dbo.RPT_COUNT_ALL_LEVEL_GRAFIK(@rbimId, @assetId)
        `;
        ...
        console.error('Error fetching report-count-all-level-graphic:', error);
        return NextResponse.json(
            {
                success: false,
