...
/**
 * GET /api/risk-analysis/report-count-all-level
 *
 * Query Parameters:
 ...
 *
 * Returns:
 * - data: All rows from dbo.RPT_COUNT_ALL_LEVEL for the given model and asset
 */
    ...
    try {
        ...
        const query = `
            SELECT
                ...
                ct_id,
                asset
            FROM dbo.RPT_COUNT_ALL_LEVEL(@rbimId, @assetId)
        `;
        ...
        console.error('Error fetching report-count-all-level:', error);
        return NextResponse.json(
            {
                success: false,
