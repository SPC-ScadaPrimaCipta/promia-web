...
/**
 * GET /api/risk-analysis/report-unit-definitions
 * 
 * Query Parameters:
 ...
 * 
 * Returns:
 * - data: All rows from dbo.RPT_UNIT_DEFINITION_L2L3 for the given model and asset
 */
export async function GET(request: NextRequest) {
    try {
        ...
        // Query: SELECT ASSET_ID, ASSET_NAME, HIERARCHY_ID, DESCRIPTION, PIPING, EQUIPMENT, level FROM dbo.RPT_UNIT_DEFINITION_L2L3(@RBIM_ID, @ASSET_ID)
        const query = `
            SELECT
                ASSET_ID,
                ASSET_NAME,
                ...
                EQUIPMENT,
                level
            FROM dbo.RPT_UNIT_DEFINITION_L2L3(@rbimId, @assetId)
        `;
        ...
        console.error('Error fetching report unit definitions:', error);
        return NextResponse.json(
            {
                success: false,
